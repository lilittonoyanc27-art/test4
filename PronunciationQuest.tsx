...
const TASKS: WordTask[] = [
  { id: 1, word: 'Rosa', type: 'double', armTranslation: 'Վարդ' }, // Initial R is always trilled
...
export default function PronunciationQuest() {
  ...
  const analyzeAudio = async (blob: Blob) => {
    ...
    try {
      ...
      The target is the letter "r". In Spanish, this is either a single tap [ɾ] or an alveolar trill [r].
      This word has a ${currentTask.type === 'double' ? 'trilled (strong) RR' : 'single tap (soft) R'}.
      Listen to the audio and evaluate:
      1. Is it the correct word?
      2. Is the "R" pronunciation accurate for this specific word?
      Return a JSON response:
      {
        "score": number (0-100),
        "success": boolean (true if score > 75),
        "text": "feedback string in Armenian language explaining what was good or what to improve regarding the Spanish R"
      }`;
      ...
      <div className="max-w-4xl w-full space-y-12 pb-20">
        ...
        {feedback && (
          <div className={`p-10 rounded-[3rem] border-4 space-y-4 shadow-2xl text-center ${feedback.success ? 'bg-emerald-500/10 border-emerald-500/20' : 'bg-rose-500/10 border-rose-500/20'}`}>
             <div className="flex items-center justify-center gap-3 text-3xl font-black uppercase italic tracking-tighter">
                {feedback.success ? (
                  <>
